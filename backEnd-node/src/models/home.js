...
let getAllTransportNumber = function (role,name) {
    if(role == 1 || role == 2){
        let sql = `SELECT count(id) as allTransportNumber  FROM transport_list WHERE operator_role >= ${role} and is_show = 1`
...
let getOrderNumber = function (role) {
    let sql = `SELECT order_status, count(id) as orderNumber  FROM order_list WHERE operator_role >= ${role} and is_show = 1 group by order_status`
...
let getTransportNumber = function (role,name) {
    if(role == 1 || role == 2){
        let sql = `SELECT transport_state, count(id) as transportNumber  FROM transport_list WHERE operator_role >= ${role} and is_show = 1 group by transport_state`
...
let getOrderDate = function (role, order_date) {
    ...
    return query(sql, [role, order_date])
}

/*获取最近七天的每天的运输单数 */
let getTransportDate = function (role,name,transport_date) {
    if(role == 1 || role == 2){
        let sql = "SELECT transport_date, count(id) as transportNumber FROM transport_list WHERE operator_role >= ? and is_show = 1 and transport_date = ?"
        // sql = "SELECT transport_date, count(id) as transportNumber FROM transport_list WHERE is_show = 1 and transport_date = ?"
        return query(sql, [role, transport_date])
    }else if(role == 3 || role == 4){
...
let getOrderDateStatus = function (role, order_date, order_status) {
    ...
    return query(sql, [role, order_date, order_status])
}

/*获取最近七天的每天的运输单数 - 运输单状态 */
let getTransportDateStatus = function (role, name, transport_date, transport_state) {
    if(role == 1 || role == 2){
        let sql = "SELECT count(id) as transportNumber FROM transport_list WHERE operator_role >= ? and is_show = 1 and transport_date = ? and transport_state = ?"
        // sql = "SELECT count(id) as transportNumber FROM transport_list WHERE is_show = 1 and order_date = ? and transport_state = ?"
        return query(sql, [role, transport_date, transport_state])
    }else if(role == 3 || role == 4){
